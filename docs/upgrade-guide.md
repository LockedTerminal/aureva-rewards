# Aureva Rewards Contract — Upgrade Guide

This guide covers the upgrade path for the standalone `contracts/aureva-rewards` Soroban contract. The contract exposes two admin-only entrypoints for this process:

The `aureva-rewards` Soroban contract supports in-place WASM upgrades via
...
4. Know the deployed contract ID for the `aureva-rewards` instance you are updating.
...
In `contracts/aureva-rewards/src/lib.rs`, inside `migrate()`, add a versioned
...
# Output: ../../target/wasm32-unknown-unknown/release/aureva_rewards.wasm
```
...
stellar contract upload \
  --wasm contracts/aureva-rewards/target/wasm32-unknown-unknown/release/aureva_rewards.optimized.wasm \
...
3. Re-run [`contracts/aureva-rewards/tests/upgrade.rs`](../contracts/aureva-rewards/tests/upgrade.rs).
