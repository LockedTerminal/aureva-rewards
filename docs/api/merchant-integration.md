...
This guide covers the complete merchant workflow on Aureva Rewards — from registering your account through distributing AUR tokens and tracking redemptions via the API. It is written for developers building merchant integrations.
...
```bash
curl -X POST https://api.aureva-rewards.io/api/merchants/register \
...
| Setting | Value |
|---------|-------|
| Base URL | `https://sandbox.aureva-rewards.io` |
...
| `reward_rate` | number | AUR tokens issued per USD of purchase amount |
...
| Field | Type | Description |
|-------|------|-------------|
| `user_id` | integer | Aureva Rewards user ID of the customer |
...
Users redeem AUR tokens through the Aureva Rewards app. When a redemption is tied to one of your campaigns, you receive a `redemption.completed` webhook event and can also query the API directly.
...
| | Testnet | Mainnet |
|-|---------|---------|
| Base URL | `https://sandbox.aureva-rewards.io` | `https://api.aureva-rewards.io` |
