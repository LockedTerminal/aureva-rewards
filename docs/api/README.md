# Aureva Rewards API Reference

**Base URL:** `http://localhost:3001/api/v1` (dev) · `https://api.AurevaRewards.io/api/v1` (prod)
...
AurevaRewards uses URL-based API versioning. The current version is `v1`.
...
{
  "success": true,
  "data": { "walletAddress": "GABC...", "nonce": "abc123", "timestamp": 1700000000, "domain": "AurevaRewards.io", "expiresAt": "2025-01-15T10:35:00Z", "message": "Sign this message..." }
}
```
...
Get the user's on-chain AUR token balance from Horizon (cached 30 s).
...
Distribute AUR tokens directly to a customer wallet on-chain.
...
Get the authenticated user's live AUR token balance from Stellar.
