...
| `insufficient_balance` | Merchant's AUR token balance is less than the requested `tokenAmount`. |
...
All Aureva Rewards Soroban contracts use `panic!` with a string message as the error mechanism.
...
| `"insufficient contract balance"` | `distribute` | Contract holds fewer tokens than the requested amount. | Fund the contract with AUR tokens before distributing. |
