# Aureva Rewards — Audit Preparation Checklist
...
| Backend auth | `AurevaRewards/backend/routes/auth.js`, `middleware/authenticateUser.js` |
| Token service | `AurevaRewards/backend/services/tokenService.js` |
...
| Deployment | `AurevaRewards/deploy/`, `scripts/deploy-contracts.sh` |
| DB migrations | `AurevaRewards/database/` |
...
- [ ] `aureva-rewards` uses `i128` for all balance × rate intermediate products to avoid overflow (`SCALE_FACTOR = 1_000_000`).
- [ ] `saturating_add` is used for balance credit operations in `aureva_token` to cap at `i128::MAX` rather than wrapping.
...
- [ ] `swap_for_xlm` in `aureva-rewards` deducts balance *before* calling the DEX router.
...
- [ ] Daily withdrawal limit (`DailyLimit`) prevents a single wallet from draining `aureva-rewards` or `reward_pool`.
- [ ] Backend rate limiters: global (100 req/min), login (10/15min), refresh (30/15min) — verify `AurevaRewards/backend/server.js` mounting.
...
- [ ] `.env` files are in `.gitignore` — verify with `git ls-files AurevaRewards/.env`.
...
- [ ] `aureva-rewards` `migrate()` panics if `migrated_version >= migration_version` (idempotency guard).
...
- [ ] Shared logic (math, events, constants) lives in `contracts/aureva-rewards/src/utils/` — not duplicated across contracts.
...
- [ ] `aureva-rewards` tests cover: initialize, set_balance, stake, unstake, claim_staking_reward, swap_for_xlm, pause/unpause, upgrade/migrate.
- [ ] `aureva_token` tests cover: initialize, mint, burn, transfer, approve, transfer_from, expired allowance.
...
- [ ] `contracts/integration_tests/` covers cross-contract interaction: campaign → distribution → aureva_token flow.
- [ ] Integration test covers the full staking lifecycle across `aureva-rewards` + `aureva_token`.
...
### 3.5 Backend Tests
- [ ] `AurevaRewards/backend/tests/` includes tests for: auth register, login, refresh, logout, password reset.
...
- [ ] `MigrationVersion == MigratedVersion` for `aureva-rewards` on each deployed network (verify with `stellar contract invoke -- get_migration_version`).
...
- [ ] `AurevaRewards/database/` migrations run cleanly on a fresh PostgreSQL instance: `npm run migrate`.
...
- [ ] Audit log retention policy (`AurevaRewards/database/021_audit_logs_retention_policy.sql`) is applied — logs retained for ≥ 90 days.
