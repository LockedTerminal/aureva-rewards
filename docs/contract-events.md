# Aureva Rewards — Contract Event Schemas
...
## 2. AurevaToken (`aur_tok`)

Source: `contracts/aureva_token/src/lib.rs`

| event_type      | topics tuple                          | data (after schema_version)                              | trigger                    |
|-----------------|---------------------------------------|----------------------------------------------------------|----------------------------|
| `mint`          | `("aur_tok", "mint")`                | `to: Address, amount: i128`                              | Tokens minted              |
| `burn`          | `("aur_tok", "burn")`                | `from: Address, amount: i128`                            | Tokens burned              |
| `transfer`      | `("aur_tok", "transfer")`            | `from: Address, to: Address, amount: i128`               | Token transfer             |
| `transfer_from` | `("aur_tok", "transfer_from")`       | `spender: Address, from: Address, to: Address, amount: i128` | Allowance-based transfer|
| `approve`       | `("aur_tok", "approve")`             | `owner: Address, spender: Address, amount: i128`         | Allowance set              |
| `inc_allow`     | `("aur_tok", "inc_allow")`           | `owner: Address, spender: Address, new_allowance: i128`  | Allowance increased        |
| `dec_allow`     | `("aur_tok", "dec_allow")`           | `owner: Address, spender: Address, new_allowance: i128`  | Allowance decreased        |

> **Note:** `aureva_token` events do not yet include `schema_version` as the first data element.
...
Every contract that implements the M-of-N upgrade mechanism emits a `ContractUpgraded` event when the WASM is swapped. The `aureva-rewards` contract additionally emits `upgraded` from `migrate()` with the migration version number.
