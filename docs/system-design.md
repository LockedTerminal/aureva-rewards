...
The diagrams cover the currently implemented Aureva Rewards platform:

- `AurevaRewards/frontend`: Next.js PWA, Freighter wallet integration, API clients,
  Zustand stores, and user/merchant views.
- `AurevaRewards/backend`: Express API, middleware, service layer, repositories,
  BullMQ workers, contract event indexing, webhooks, and observability.
- `AurevaRewards/database`: PostgreSQL migrations for merchants, users, campaigns,
  reward issuances, point transactions, redemptions, webhooks, and contract
  events.
- `AurevaRewards/blockchain`: Stellar Horizon payment and trustline helpers.
- `contracts`: Soroban contracts in the workspace.
- `helm`, `k8s`, `infra`, and `AurevaRewards/docker-compose*.yml`: deployment
...
flowchart LR
    ...
    subgraph frontend["Frontend: AurevaRewards/frontend"]
        next["Next.js PWA"]
    ...
    subgraph backend["Backend: AurevaRewards/backend"]
    ...
    subgraph contracts["Soroban contracts: contracts/*"]
        AurevaToken["aureva_token"]
        rewardPool["reward_pool"]
        distribution["distribution"]
        AurevaRewards["aureva-rewards"]
        campaign["campaign"]
...
sequenceDiagram
    ...
    alt Duplicate idempotency key
        ...
        alt Campaign inactive or expired
            ...
            Worker->>Trustline: verify recipient AUR trustline
            ...
            alt Missing trustline or invalid payment state
                ...
                Horizon->>Ledger: apply AUR payment transaction
...
flowchart LR
    ...
    sorobanRpc --> campaign
    backendPayments -->|"verify trustline, submit AUR payment"| horizon
    horizon --> ledger
    ledger --> AurevaToken
    freighterWallet -->|"wallet auth, trustline, user-signed tx"| ledger

    rewardPool -->|"transfer(from, pool, amount)"| AurevaToken
    rewardPool -->|"balance(pool)"| AurevaToken
    rewardPool -->|"transfer(pool, recipient, amount)"| AurevaToken

    distribution -->|"balance(distribution)"| AurevaToken
    distribution -->|"transfer(distribution, recipient, amount)"| AurevaToken
    distribution -->|"transfer_from(distribution, recipient, distribution, amount)"| AurevaToken

    AurevaRewards -->|"swap_exact_in(user, amount, min_out, path)"| dexRouter
    AurevaRewards -.->|"stores XLM SAC address for swap path"| xlmSac

    campaign -.->|"stores reward_token address; emits reward event"| AurevaToken
    referral -.->|"internal pool accounting in current code"| AurevaToken
    vesting -.->|"internal pool accounting in current code"| AurevaToken
    redemption -.->|"request/confirm/cancel lifecycle only"| AurevaToken
    adminRoles -.->|"authorizes privileged operations"| AurevaRewards
    governance -.->|"proposal execution placeholder"| adminRoles
    escrow -.->|"escrow lifecycle only"| AurevaToken
    contractState -.->|"state snapshots and migrations"| AurevaRewards
...
| --- | --- | --- | --- |
| `reward_pool` | `aureva_token` | `contracts/reward_pool/src/lib.rs` | `transfer`, `balance` |
| `distribution` | `aureva_token` | `contracts/distribution/src/lib.rs` | `balance`, `transfer`, `transfer_from` |
| `aureva-rewards` | configured DEX router | `contracts/aureva-rewards/src/lib.rs` | `swap_exact_in` |
| Backend Soroban service | `campaign` contract | `AurevaRewards/backend/services/sorobanService.js` | `register_campaign`, `update_campaign`, `pause_campaign` |
| Backend payment helper | Stellar ledger / AUR asset | `AurevaRewards/blockchain/sendRewards.js` | Horizon payment operation |
...
flowchart TB
    ...
    subgraph staging["Staging: AurevaRewards/docker-compose.staging.yml"]
    ...
    subgraph prodK8s["Production app runtime: helm/aureva-rewards"]
        ...
        k8sSecrets["aureva-rewards-secrets"]
...
| --- | --- | --- |
| Runtime | Docker Compose from `AurevaRewards/docker-compose.staging.yml` | Helm chart in `helm/aureva-rewards` on AWS-managed infrastructure |
