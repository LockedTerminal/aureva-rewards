...
This document describes the storage layout optimizations applied to Aureva Rewards smart contracts to stay within Soroban's WASM size and storage budget limits.

## Optimization Strategy

This audit reviewed the `aureva-rewards`, `aureva_token`, `vesting`, `referral`, `reward_pool`, and `admin_roles` contracts. Existing keys are already mostly consolidated; the new `DailyUsage` struct in `reward_pool` further reduces per-wallet storage overhead.
...
### Aureva Rewards Contract (`aureva-rewards`)
...
### AUR token Contract (`aureva_token`)
...
### Temporary Storage
- Not currently used in Aureva Rewards contracts
...
stellar contract inspect --wasm target/wasm32v1-none/release/aureva_rewards.wasm
stellar contract inspect --wasm target/wasm32v1-none/release/aureva_token.wasm
