...
node AurevaRewards/scripts/encrypt-existing-rows.js --rotate
```
...
`AurevaRewards/backend/lib/encryption.js`
...
`AurevaRewards/backend/lib/prismaEncryptionMiddleware.js`
...
- `AurevaRewards/backend/db/userRepository.js` — `encryptEmail` / `decryptUserRow`
- `AurevaRewards/backend/db/webhookRepository.js` — `encryptWebhookSecret` / `decryptWebhookRow`
- `AurevaRewards/backend/routes/auth.js` — encrypts email on register/login before DB lookup
...
- `AurevaRewards/backend/tests/auth.test.js` (email encryption through register/login)
- `AurevaRewards/backend/tests/security/security.test.js` (field-level encryption assertions)
