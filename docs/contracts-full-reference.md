# Aureva Rewards — Full Contract Reference

> Closes #387

This document is the authoritative reference for all Soroban smart contracts in the Aureva Rewards platform. Each section covers: purpose, state variables, public functions, events, errors, usage examples, and security considerations.
...
2. [Aureva Rewards Contract](#2-aureva-rewards-contract)
3. [AUR token Contract](#3-AUR-token-contract)
...
## 2. Aureva Rewards Contract

**Source:** `contracts/aureva-rewards/src/lib.rs`
...
| `AurevaToken` | `Address` | Address of the AUR token contract |
...
#### `initialize(admin, aureva_token, event_config)`
...
```rust
client.initialize(&admin, &aureva_token_addr, &EventConfig { schema_version: 1 });
...
## 3. AUR token Contract

**Source:** `contracts/aureva_token/src/lib.rs`
...
| Topics | Data | Trigger |
|--------|------|---------|
| `("aur_tok", "mint")` | `(to, amount)` | `mint` |
| `("aur_tok", "burn")` | `(from, amount)` | `burn` |
| `("aur_tok", "transfer")` | `(from, to, amount)` | `transfer` / `transfer_from` |
| `("aur_tok", "approve")` | `(owner, spender, amount, expiration_ledger)` | `approve` |
...
|-----|------|-------------|
| `Admin` | `Address` | Contract admin |
| `AurevaToken` | `Address` | AUR token contract address |
...
| Function | Auth | Description |
|----------|------|-------------|
| `initialize(admin, aureva_token)` | — | One-time setup |
...
Issues, tracks, and processes AUR token redemption requests with per-campaign expiry windows.
...
### Pattern A — Two-Step Admin Upgrade (aureva-rewards)

Used by `aureva-rewards`. Separates the WASM swap from data migration.
...
cd contracts/aureva-rewards
cargo build --release --target wasm32v1-none
# Output: ../../target/wasm32v1-none/release/aureva_rewards.wasm

# 2. Upload the new WASM to the network
stellar contract upload \
  --source ADMIN_SECRET \
  --network testnet \
  --wasm ../../target/wasm32v1-none/release/aureva_rewards.wasm
...
```rust
// In contracts/aureva-rewards/src/lib.rs, inside migrate():
...
# Check migration versions match (aureva-rewards only)
