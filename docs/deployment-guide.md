# Aureva Rewards — Deployment Guide

This guide covers end-to-end deployment of all Aureva Rewards components — Soroban smart contracts, Node.js backend, and Next.js frontend — for both testnet and mainnet environments.
...
```bash
git clone https://github.com/barry01-hash/aureva-rewards.git
cd aureva-rewards
```
...
```bash
cp AurevaRewards/.env.example AurevaRewards/.env
```
...
| `PORT` | `3001` | `3001` | |
| `DATABASE_URL` | `postgresql://AUR:changeme@localhost:5432/aureva_rewards` | Managed DB connection string | Never commit |
...
| `ALLOWED_ORIGIN` | `http://localhost:3000` | `https://aureva-rewards.xyz` | |
| `aureva_token_CONTRACT_ID` | Output from contract deploy | Output from contract deploy | |
| `REWARD_POOL_CONTRACT_ID` | Output from contract deploy | Output from contract deploy | |

See `AurevaRewards/.env.example` for the full variable reference including email, rate limiting, and backup settings.

### 2.2 Frontend Environment Variables

Frontend build-time variables are set in Vercel's project settings (not in a committed file). For local development, create `AurevaRewards/frontend/.env.local`:

```bash
# AurevaRewards/frontend/.env.local — not committed to git
...
|-------|----------|----------------|
| 1 | `aureva_token` | `aureva_token_CONTRACT_ID` |
...
Update `aureva_token_CONTRACT_ID` and `REWARD_POOL_CONTRACT_ID` in the production backend `.env` (or secrets manager) with the values from `.env.mainnet`.
...
```bash
ssh deploy@aureva-rewards.xyz
cd /opt/aureva-rewards
```

**Step 2 — Take a database backup before deploying:**

```bash
docker compose exec postgres pg_dump -U AUR aureva_rewards \
...
```bash
curl -f https://api.aureva-rewards.xyz/health
...
| Variable | Value |
|----------|-------|
| `NEXT_PUBLIC_API_URL` | `https://api-preview.aureva-rewards.xyz` |
| `NEXT_PUBLIC_HORIZON_URL` | `https://horizon-testnet.stellar.org` |
...
| Variable | Value |
|----------|-------|
| `NEXT_PUBLIC_API_URL` | `https://api.aureva-rewards.xyz` |
...
# Production
GATEWAY_URL=https://api.aureva-rewards.xyz bash scripts/gateway-smoke-test.sh
```
...
```bash
node AurevaRewards/scripts/check-balances.js
```
...
```bash
# Verify aureva_token admin is set correctly
stellar contract invoke \
  --id $aureva_token_CONTRACT_ID \
  --network-passphrase "Public Global Stellar Network ; September 2015" \
  --rpc-url https://soroban-rpc.stellar.org \
...
docker compose exec -T postgres psql -U AUR aureva_rewards \
...
Or change the host port mapping in `AurevaRewards/docker-compose.yml`.
...
Build-time variables must be present at `next build` time. For local builds, ensure `AurevaRewards/frontend/.env.local` exists. For Vercel, confirm the variable is set in **Project → Settings → Environment Variables** for the correct scope (Preview or Production).
