# Icon Guidelines — Aureva Rewards
...
Aureva Rewards uses **[Heroicons](https://heroicons.com/)** as the primary icon library.
...
These icons are specific to the Aureva Rewards brand and are not in Heroicons. Store them in `public/icons/` as SVG and create React wrappers in `components/icons/`.

### AUR token Coin

Represents the AUR token currency. Used in:
- Balance displays
- Reward amounts
- Token metrics

```jsx
// components/icons/AurevaTokenIcon.jsx
export function AurevaTokenIcon({ className = 'w-6 h-6', ...props }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
