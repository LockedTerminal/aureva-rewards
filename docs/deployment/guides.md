...
This document covers the full deployment lifecycle for Aureva Rewards — from local prerequisites through production release and rollback.
...
All variables are defined in `AurevaRewards/.env.example`. Copy it to `AurevaRewards/.env` for local development — **never commit `.env` to source control**.
...
|----------|--------------|-----------------|-------------|
| `NEXT_PUBLIC_API_URL` | `https://api-preview.aureva-rewards.xyz` | `https://api.aureva-rewards.xyz` | Backend API base URL |
| `NEXT_PUBLIC_HORIZON_URL` | `https://horizon-testnet.stellar.org` | `https://horizon.stellar.org` | Stellar Horizon endpoint |
| `NEXT_PUBLIC_ISSUER_PUBLIC` | Testnet key (`G...`) | Mainnet key (`G...`) | AUR asset issuer public key |
...
| `aureva_token_CONTRACT_ID` | ✅ | Soroban AUR token contract ID |
...
ssh deploy@staging.aureva-rewards.xyz
...
```bash
# Health check
curl https://api-preview.aureva-rewards.xyz/health
...
# 6. Verify health
curl https://api.aureva-rewards.xyz/health
bash scripts/gateway-smoke-test.sh
```
...
- All migrations in `AurevaRewards/database/` are numbered sequentially and append-only.
...
```bash
docker compose exec postgres pg_dump -U AUR aureva_rewards > backup-$(date +%Y%m%d-%H%M).sql
```
