...
**Cause:** `AurevaRewards/.env` was not created, or required variables were left as placeholders.
...
|----------|----------------|
| `JWT_PRIVATE_KEY` / `JWT_PUBLIC_KEY` | `node AurevaRewards/backend/scripts/generate-jwt-keys.js` |
...
cd AurevaRewards/backend && npm install
cd AurevaRewards/frontend && npm install
```
...
node AurevaRewards/backend/scripts/generate-jwt-keys.js
...
### 3.4 `Error: Stellar account has no trustline for AUR`

**Symptom:** Reward issuance or token transfer fails because the recipient wallet has no AUR trustline.

**Cause:** The recipient Stellar account has not established a trustline for the AUR asset.
...
node AurevaRewards/database/migrate.js
```
...
node AurevaRewards/database/migrate.js --status
...
### 4.4 `password authentication failed for user "AUR"`
...
docker compose logs > aureva-rewards-logs.txt 2>&1
```
