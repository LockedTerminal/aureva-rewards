...
This document outlines the storage optimization strategy for Aureva Rewards smart contracts to minimize ledger entry fees on Stellar. Each data key is classified by storage type and includes TTL extension justification.
...
- `aureva_token`: User balances, allowances
...
```rust
// Example: aureva_token balance_of
...
- [x] Admin address → Instance storage (no TTL needed)
- [x] AUR token address → Instance storage (no TTL needed)
...
- AUR token: ~100 bytes per balance entry (persistent)
...
- AUR token: ~100 bytes per balance (no change, but TTL extended in hot paths)
