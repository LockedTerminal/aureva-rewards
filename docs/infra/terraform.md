...
aws s3api list-object-versions \
  --bucket aureva-rewards-tf-state \
  --prefix aureva-rewards/terraform.tfstate

# Restore a previous version (replace VERSION_ID)
aws s3api copy-object \
  --bucket aureva-rewards-tf-state \
  --copy-source "aureva-rewards-tf-state/aureva-rewards/terraform.tfstate?versionId=VERSION_ID" \
  --key aureva-rewards/terraform.tfstate
