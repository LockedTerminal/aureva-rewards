...
- `aureva_token` owns fungible token balances and allowances.
- `reward_pool` calls `aureva_token` for pool deposits, balance checks, and
  withdrawals.
- `distribution` calls `aureva_token` for distribution and clawback transfers.
- `aureva-rewards` owns staking, account recovery, upgrade hooks, and configured
  DEX-router swaps.
...
- Code: `contracts/aureva_token/src/lib.rs`
- Code: `contracts/reward_pool/src/lib.rs`
- Code: `contracts/distribution/src/lib.rs`
- Code: `contracts/aureva-rewards/src/lib.rs`
