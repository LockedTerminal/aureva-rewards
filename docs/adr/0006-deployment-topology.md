...
- Staging uses `AurevaRewards/docker-compose.staging.yml` with Nginx, frontend,
  backend, PostgreSQL, Redis, and one-shot migrations.
- Production uses the Helm chart in `helm/aureva-rewards` for frontend/backend
...
- Code: `AurevaRewards/docker-compose.staging.yml`
- Code: `AurevaRewards/docker-compose.prod.yml`
- Code: `helm/aureva-rewards`
- Code: `infra`
- Code: `monitoring`
