...
Aureva Rewards must track merchants, users, campaigns, transactions, point
...
- PostgreSQL stores durable business state through SQL migrations in
  `AurevaRewards/database`.
- Repository modules in `AurevaRewards/backend/db` own database access.
...
- Code: `AurevaRewards/database`
- Code: `AurevaRewards/backend/db`
- Code: `AurevaRewards/backend/jobs/queues.js`
