...
All Aureva Rewards smart contracts are deployed on the Stellar network using the Soroban runtime.
...
|----------|------------|----------------------|---------|
| `aureva-rewards` | `CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX` | — | 1.0.0 |
| `aureva_token` | `CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX` | — | 1.0.0 |
...
| Contract | Contract ID | Deployed At (Ledger) | Version |
|----------|------------|----------------------|---------|
| `aureva-rewards` | `CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX` | — | — |
| `aureva_token` | `CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX` | — | — |
...
stellar contract deploy \
  --wasm target/wasm32-unknown-unknown/release/aureva_token.wasm \
  --source deployer \
  --network testnet

# Deploy aureva-rewards
stellar contract deploy \
  --wasm target/wasm32-unknown-unknown/release/aureva_rewards.wasm \
  --source deployer \
  --network testnet
```
...
stellar contract invoke \
  ...
  -- initialize \
  --admin <ADMIN_ADDRESS> \
  --token_id <aureva_token_CONTRACT_ID>
```
...
aureva_token           (standalone)
reward_pool          (standalone)
vesting              (standalone)
referral             (standalone)
governance           (standalone)
distribution    ──►  aureva_token  (calls transfer / transfer_from)
aureva-rewards    ──►  DEX router  (calls swap_exact_in via set_swap_config)
```
