...
This document describes how to rotate every secret used by Aureva Rewards.
...
| `aureva_token_CONTRACT_ID` | Backend | GitHub Actions Secret |
...
> **Impact:** Rotating the issuer keypair requires re-issuing the AUR asset from
...
3. Re-run the asset setup script: `node AurevaRewards/scripts/setup.js --use-env`.
...
1. Fund the new distribution account and establish a trustline to the AUR asset.
2. Transfer remaining AUR balance from old distribution account to new one.
...
1. Connect to the PostgreSQL instance and change the password:
   ```sql
   ALTER USER AUR WITH PASSWORD '<new-password>';
   ```
...
gh secret set SECRET_NAME --body "secret-value" --repo org/aureva-rewards

# For environment-specific secrets (staging / production)
gh secret set SECRET_NAME --body "secret-value" \
  --repo org/aureva-rewards \
  --env production
```
...
Aureva Rewards encrypts PII (e.g. `users.email`, `webhooks.secret`) with AES-256-GCM, keyed by
...
FIELD_ENCRYPTION_KEY=<CURRENT_KEY> \
  node AurevaRewards/scripts/encrypt-existing-rows.js \
    --new-key <NEW_KEY> \
    --old-key <CURRENT_KEY> \
    --batch-size 200 \
    --dry-run
...
```bash
# GitHub Actions
gh secret set FIELD_ENCRYPTION_KEY             --body "$NEW_KEY"       --repo org/aureva-rewards
gh secret set FIELD_ENCRYPTION_KEY_PREVIOUS    --body "$CURRENT_KEY"   --repo org/aureva-rewards
gh secret set FIELD_ENCRYPTION_KEY_ROTATED_AT  --body "$(date -u +'%Y-%m-%dT%H:%M:%SZ')" --repo org/aureva-rewards
...
```bash
kubectl rollout restart deployment/AUR-backend
# or
...
```bash
# GitHub Actions
gh secret delete FIELD_ENCRYPTION_KEY_PREVIOUS   --repo org/aureva-rewards
gh secret delete FIELD_ENCRYPTION_KEY_ROTATED_AT --repo org/aureva-rewards
