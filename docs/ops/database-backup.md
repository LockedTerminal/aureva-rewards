...
export BACKUP_S3_BUCKET="aureva-rewards-backups"
export BACKUP_PASSPHRASE="$(aws secretsmanager get-secret-value \
  --secret-id aureva-rewards/production/backup-passphrase \
...
```bash
BACKUP_FILE="aureva_rewards_20260424T020000Z.dump.enc"
...
```bash
createdb -h <host> -U <admin_user> aureva_rewards_restored
pg_restore "${RESTORE_DIR}/restore.dump" \
  --dbname="postgresql://<admin_user>:<pass>@<host>/aureva_rewards_restored" \
