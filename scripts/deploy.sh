#!/usr/bin/env bash
# deploy.sh — Deploy all Aureva Rewards contracts to testnet or mainnet.
#
# Usage:
...
# ── Deployment order ──────────────────────────────────────────────────────────
# 1. aureva_token  (no deps)
# 2. distribution (needs aureva_token contract ID)
# 3. reward_pool, vesting, referral, aureva_rewards (no inter-deps)
# 4. admin_roles (last — governs all others)

# 1. aureva_token
aureva_token_ID="$(deploy_contract \
  "aureva_token_contract_id" "aureva_token" \
  --admin "${ADMIN_ADDRESS}")"

# 2. distribution (depends on aureva_token)
DISTRIBUTION_ID="$(deploy_contract \
  "distribution_contract_id" "distribution" \
  --admin "${ADMIN_ADDRESS}" \
  --token-id "${aureva_token_ID}")"
...
# 3d. aureva_rewards
aureva_rewards_ID="$(deploy_contract \
  "aureva_rewards_contract_id" "aureva_rewards" \
  --admin "${ADMIN_ADDRESS}")"
