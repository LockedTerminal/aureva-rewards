#!/usr/bin/env bash
# verify-deployment.sh — Post-deployment verification for all Aureva Rewards contracts.
#
...
aureva_token_ID="$(json_get "aureva_token_contract_id")"
...
aureva_rewards_ID="$(json_get "aureva_rewards_contract_id")"
...
log "========================================================"
log "Aureva Rewards — Post-Deployment Verification"
log "Network  : ${NETWORK}"
log "RPC      : ${RPC_URL}"
...
# ── 1. aureva_token ─────────────────────────────────────────────────────────────
log ""
log "── aureva_token (${aureva_token_ID:-<missing>}) ──"

if require_id "aureva_token" "$aureva_token_ID"; then
  # Deployed & callable
  if invoke_ro "$aureva_token_ID" "balance" --addr "$ADMIN_ADDRESS"; then
    ok "aureva_token: deployed and callable (balance)"
    assert_is_integer "aureva_token.balance"
  else
    fail "aureva_token: balance call failed — ${LAST_ERROR}"
  fi

  # Admin balance query returns a non-negative integer (0 is fine on fresh deploy)
  if invoke_ro "$aureva_token_ID" "balance" --addr "$ADMIN_ADDRESS"; then
    aureva_token_bal="${LAST_OUTPUT//\"/}"
    if [[ "$aureva_token_bal" =~ ^[0-9]+$ ]]; then
      ok "aureva_token: admin balance is a non-negative integer (${aureva_token_bal})"
    else
      fail "aureva_token: unexpected balance value '${aureva_token_bal}'"
    fi
  fi

  # Allowance query (zero is expected on fresh deploy)
  if invoke_ro "$aureva_token_ID" "allowance" \
      --owner "$ADMIN_ADDRESS" --spender "$ADMIN_ADDRESS"; then
    ok "aureva_token: allowance query callable"
    assert_is_integer "aureva_token.allowance"
  else
    fail "aureva_token: allowance call failed — ${LAST_ERROR}"
  fi
fi
...
# ── 7. aureva_rewards ───────────────────────────────────────────────────────────
log ""
log "── aureva_rewards (${aureva_rewards_ID:-<missing>}) ──"

if require_id "aureva_rewards" "$aureva_rewards_ID"; then
  # is_paused should be false on fresh deploy
  if invoke_ro "$aureva_rewards_ID" "is_paused"; then
    paused="${LAST_OUTPUT//\"/}"
    if [[ "$paused" == "false" ]]; then
      ok "aureva_rewards.is_paused: contract is not paused"
    else
      fail "aureva_rewards.is_paused: expected 'false', got '${paused}'"
    fi
  else
    fail "aureva_rewards: is_paused call failed — ${LAST_ERROR}"
  fi

  # get_annual_rate returns 0 on fresh deploy (not yet configured)
  if invoke_ro "$aureva_rewards_ID" "get_annual_rate"; then
    ok "aureva_rewards: get_annual_rate callable"
    assert_is_integer "aureva_rewards.get_annual_rate"
  else
    fail "aureva_rewards: get_annual_rate call failed — ${LAST_ERROR}"
  fi

  # get_balance for admin should be 0 on fresh deploy
  if invoke_ro "$aureva_rewards_ID" "get_balance" --user "$ADMIN_ADDRESS"; then
    ok "aureva_rewards: get_balance callable"
    assert_is_integer "aureva_rewards.get_balance"
  else
    fail "aureva_rewards: get_balance call failed — ${LAST_ERROR}"
  fi

  # get_migration_version should be 0 on fresh deploy
  if invoke_ro "$aureva_rewards_ID" "get_migration_version"; then
    ok "aureva_rewards: get_migration_version callable"
    assert_is_integer "aureva_rewards.get_migration_version"
  else
    fail "aureva_rewards: get_migration_version call failed — ${LAST_ERROR}"
  fi

  # get_recovery_admin should return a valid address
  if invoke_ro "$aureva_rewards_ID" "get_recovery_admin"; then
    ok "aureva_rewards: get_recovery_admin callable"
    assert_is_address "aureva_rewards.get_recovery_admin"
  else
    fail "aureva_rewards: get_recovery_admin call failed — ${LAST_ERROR}"
  fi
fi
