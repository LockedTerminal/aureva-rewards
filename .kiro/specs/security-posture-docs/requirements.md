...
3. **`docs/security/merchant-security-guide.md`** — security best practices for merchants integrating the Aureva Rewards API
...
- **Aureva_Rewards_Platform**: The full Aureva Rewards system, comprising Soroban smart contracts on Stellar, the Node.js backend API, and the Next.js frontend.
- **SECURITY_md**: The `SECURITY.md` file at the repository root, serving as the canonical responsible disclosure policy.
- **Merchant**: A business or developer integrating the Aureva Rewards API to issue or manage rewards for their customers.
- **Integrator**: Any external party (merchant, partner, or developer) consuming the Aureva Rewards API or SDK.
- **Responsible_Disclosure_Policy**: The documented process by which external security researchers report vulnerabilities to the Aureva Rewards team.
...
- **API_Key**: A credential issued to a merchant to authenticate requests to the Aureva Rewards API.
...
**User Story:** As a security researcher, I want a clear responsible disclosure policy at the repository root, so that I know how to report vulnerabilities to the Aureva Rewards team safely and what to expect in return.

#### Acceptance Criteria

1. THE Aureva_Rewards_Platform SHALL provide a `SECURITY.md` file at the repository root.
...
8. THE SECURITY_md SHALL state that the Aureva Rewards team commits to not pursuing legal action against researchers who follow the responsible disclosure policy.
...
**User Story:** As a merchant evaluating Aureva Rewards for enterprise adoption, I want to see the status and findings of smart contract audits, so that I can assess the security maturity of the platform before integrating.

#### Acceptance Criteria

1. THE Aureva_Rewards_Platform SHALL provide a `docs/audits/findings-summary.md` file.
...
**User Story:** As a merchant integrating the Aureva Rewards API, I want a dedicated security guide, so that I can configure my integration securely and protect my customers' data and my own credentials.

#### Acceptance Criteria

1. THE Aureva_Rewards_Platform SHALL provide a `docs/security/merchant-security-guide.md` file.
...
8. THE Merchant_Security_Guide SHALL include a section on dependency security, recommending that merchants using the Aureva Rewards SDK pin the SDK version and review the changelog before upgrading.
...
1. THE Aureva_Rewards_Platform SHALL provide a `docs/security/known-limitations.md` file.
...
1. THE Aureva_Rewards_Platform SHALL require Security_Lead approval before any of the four security documents (SECURITY_md, Findings_Summary, Merchant_Security_Guide, Known_Limitations_Document) is merged to the main branch.
2. THE Aureva_Rewards_Platform SHALL record the reviewer's name, approval date, and document version in a review log section within each document.
3. WHEN a security document is updated after initial approval, THE Aureva_Rewards_Platform SHALL require a new Security_Lead review before the update is merged.
4. THE Aureva_Rewards_Platform SHALL link all four new documents from the existing `docs/security/README.md` index so that they are discoverable from the security documentation hub.
5. THE Aureva_Rewards_Platform SHALL link the `SECURITY.md` from the repository's `README.md` so that it is visible to anyone visiting the repository.
