...
//! - get_balance: reflects real AUR token balance
...
// Mirrors the interface of aureva_token: initialize, mint, balance, transfer.

#[contract]
pub struct MockAurevaToken;

#[contractimpl]
impl MockAurevaToken {
...
fn setup() -> TestSetup {
    ...
    // Deploy mock AUR token
    let token_id = env.register(MockAurevaToken, ());
...
/// Mints `amount` AUR tokens to `recipient` via the mock token contract.
...
/// Reads the AUR token balance of `addr` via the mock token contract.
