...
pub enum DataKey {
    ...
    Admin,
    /// Address of the AUR token contract (SEP-41 / Soroban token interface).
    Token,
...
impl RewardPoolContract {
    ...
    /// - `token` – Address of the AUR token contract.
    ///
    /// # Errors
    ...
    /// Deposits AUR tokens from the caller into the reward pool.
    ///
    /// # Parameters
    /// - `from` – Address transferring tokens into the pool.
    /// - `amount` – Number of AUR tokens to deposit. Must be > 0.
    ///
    /// # Authorization
    ...
    /// Withdraws AUR tokens from the pool to a recipient. Admin only.
    ///
    ...
    /// - `amount` – Gross number of AUR tokens to withdraw. Must be > 0.
    ///
    /// # Authorization
    /// Requires admin authorization.
    ///
    ...
    /// Returns the pool's current AUR token balance (via live token contract call).
    ...
    /// Returns the treasury's current AUR token balance.
    ///
