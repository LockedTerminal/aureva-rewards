...
//! - [`transfer_from`](AurevaToken::transfer_from) support for allowance-based transfers
...
impl AurevaToken {
    ...
    ///
    /// # Parameters
    /// - `admin` – Address that will be authorized to call [`mint`](AurevaToken::mint).
    ///
    /// # Panics
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "mint")` with data `(to: Address, amount: i128)`.
    ///
    /// # Panics
    ...
    pub fn mint(env: Env, to: Address, amount: i128) {
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("mint")),
            (to, amount),
        );
    }
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "burn")` with data `(from: Address, amount: i128)`.
    ///
    /// # Panics
    /// - `"amount must be positive"` if `amount <= 0`.
    ...
    pub fn burn(env: Env, from: Address, amount: i128) {
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("burn")),
            (from, amount),
        );
    }
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "transfer")` with data `(from: Address, to: Address, amount: i128)`.
    ///
    /// # Panics
    /// - `"amount must be positive"` if `amount <= 0`.
    /// - `"insufficient balance"` if `from` holds fewer tokens than `amount`.
    pub fn transfer(env: Env, from: Address, to: Address, amount: i128) {
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("transfer")),
            (from, to, amount),
        );
    }
    ...
    /// via [`approve`](AurevaToken::approve). Expired allowances are treated as zero.
    ///
    /// # Parameters
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "xfer_from")` with data `(spender, from, to, amount)`.
    ///
    /// # Panics
    /// - `"amount must be positive"` if `amount <= 0`.
    ...
    pub fn transfer_from(env: Env, spender: Address, from: Address, to: Address, amount: i128) {
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("xfer_from")),
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "approve")` with data `(owner, spender, amount, expiration_ledger)`.
    ///
    /// # Panics
    ...
    pub fn approve(
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("approve")),
    ...
    /// `expiration_ledger` must be supplied via a fresh [`approve`](AurevaToken::approve) call.
    ///
    /// # Parameters
    /// - `owner` – Token owner.
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "inc_allow")` with data `(owner, spender, new_allowance)`.
    ///
    /// # Panics
    /// - `"amount must be positive"` if `amount <= 0`.
    ...
    pub fn increase_allowance(env: Env, owner: Address, spender: Address, amount: i128) {
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("inc_allow")),
    ...
    ///
    /// # Events
    /// Emits `("aur_tok", "dec_allow")` with data `(owner, spender, new_allowance)`.
    ///
    /// # Panics
    /// - `"amount must be positive"` if `amount <= 0`.
    pub fn decrease_allowance(env: Env, owner: Address, spender: Address, amount: i128) {
        ...
        env.events().publish(
            (symbol_short!("aur_tok"), symbol_short!("dec_allow")),
            (owner, spender, new_amount),
        );
    }
...
mod tests {
    ...
    fn setup() -> (Env, Address, AurevaTokenClient<'static>) {
        let env = Env::default();
        env.mock_all_auths();
        let id = env.register(AurevaToken, ());
        let client = AurevaTokenClient::new(&env, &id);
