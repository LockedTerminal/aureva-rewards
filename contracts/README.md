# Aureva Rewards — Smart Contract Ecosystem
...
2. [Contract Reference](#contract-reference)
   - [AurevaToken](#1-AurevaToken)
   - [RewardPool](#2-rewardpool)
   - [ClaimDistribution](#3-claimdistribution)
   - [Staking (aurevaRewardsContract)](#4-staking--aurevaRewardscontract)
...
The Aureva Rewards smart contract ecosystem is a modular, Soroban-based loyalty platform deployed on the Stellar network. Each contract owns a single responsibility; cross-contract interactions are kept explicit and minimal to reduce attack surface.

```
┌─────────────────────────────────────────────────────────────────┐
│                        Aureva Rewards System                      │
│                                                                 │
│  ┌──────────────┐   mint/burn/transfer   ┌──────────────────┐  │
│  │  AurevaToken   │◄──────────────────────►│   RewardPool     │  │
│  └──────────────┘                        └──────────────────┘  │
│         ▲                                        ▲              │
...
**Source:** `aureva_token/src/lib.rs`

#### Purpose

ERC-20-style fungible token on Soroban. Manages balances and allowances for the AUR reward token. Mint is admin-gated; burn and transfer are caller-gated.
...
|-----------|--------|------|
| `mint` | `("aur_tok", "mint")` | `(to: Address, amount: i128)` |
| `burn` | `("aur_tok", "burn")` | `(from: Address, amount: i128)` |
| `transfer` | `("aur_tok", "transfer")` | `(from: Address, to: Address, amount: i128)` |
| `approve` | `("aur_tok", "approve")` | `(owner: Address, spender: Address, amount: i128)` |
...
Custodial treasury that holds AUR tokens earmarked for reward distributions. Acts as the funding source for `ClaimDistribution` and `Vesting` payouts. Deposit and withdrawal are admin-gated to prevent unauthorised draining.
...
**Source:** Implemented within the `aureva-rewards` module; claim logic is coordinated between `RewardPool` and on-chain Merkle proof verification.

#### Purpose

Enables users to claim pre-allocated Aureva Rewards by submitting a valid Merkle proof. Prevents double-claims via a per-address claimed flag. On successful verification, instructs `RewardPool` to transfer tokens and emits a `claimed` event consumed by the backend webhook.
...
### 4. Staking / aurevaRewardsContract

**Source:** `aureva-rewards/src/lib.rs`

#### Purpose

Allows users to lock AUR tokens and earn time-proportional yield. The annual rate is set in basis points by the admin. Yield is calculated using fixed-point arithmetic (`SCALE_FACTOR = 1_000_000`, `SECONDS_PER_YEAR = 31_536_000`) to avoid rounding errors. Only one active stake per address is permitted at a time.
...
|-----|------|-------------|-------------|
| `DataKey::Admin` | `Address` | Instance | Contract administrator |
| `DataKey::Balance(Address)` | `i128` | Instance | AUR balance per account |
...
|---------|---------|---------|-------------|
| AurevaToken | `"aur_tok"` | `"mint"` | `(to, amount)` |
| AurevaToken | `"aur_tok"` | `"burn"` | `(from, amount)` |
| AurevaToken | `"aur_tok"` | `"transfer"` | `(from, to, amount)` |
| AurevaToken | `"aur_tok"` | `"approve"` | `(owner, spender, amount)` |
...
On-chain referral registry. Each wallet can be referred exactly once. Tracks referral counts per referrer for leaderboard purposes. The admin credits referrers with Aureva Rewards drawn from an internal pool.
...
**Source:** `aureva-rewards/src/lib.rs` (`swap_for_xlm`)

#### Purpose

Burns AUR points for the caller and routes them through a configured DEX router contract to receive XLM (or another output asset). Enforces a slippage guard (`min_xlm_out`) and caps multi-hop paths at 5 intermediate assets per Stellar protocol limits.
...
| `DataKey::Router` | `Address` | Instance | DEX router contract address |
| `DataKey::Balance(Address)` | `i128` | Instance | AUR balance (shared with Staking) |
...
erDiagram
    ...
    USER_ADDRESS ||--o{ aureva_token : "Balance / Allowance"
    ...
    aureva_token }o--|| ADMIN_ROLES : "mint gated by admin"
```
...
The `aurevaRewardsContract` supports live WASM upgrades via Soroban's `update_current_contract_wasm`.
