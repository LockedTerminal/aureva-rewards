...
impl DistributionContract {
    ...
    /// One-time setup. `token_id` is the AUR token contract address.
    ///
    /// # Parameters
    /// - `admin` – Address authorized to call distribution and clawback functions.
    /// - `token_id` – Address of the AUR token contract used for transfers.
    ...
    /// Returns the AUR token balance held by this contract.
