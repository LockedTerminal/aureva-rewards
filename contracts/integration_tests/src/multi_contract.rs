...
use aureva_rewards::{EventConfig, aurevaRewardsContract, aurevaRewardsContractClient};
...
struct TestHarness<'a> {
    ...
    rewards: aurevaRewardsContractClient<'a>,
}
...
impl<'a> TestHarness<'a> {
    fn setup() -> Self {
        ...
        // Deploy Token
        let token_id = env.register_contract(None, AurevaToken);
        let token = AurevaTokenClient::new(&env, &token_id);
        token.initialize(&admin);

        // Deploy Rewards
        let rewards_id = env.register_contract(None, aurevaRewardsContract);
        let rewards = aurevaRewardsContractClient::new(&env, &rewards_id);
        let aureva_token = Address::generate(&env);
        let event_config = EventConfig { schema_version: 1 };
        rewards.initialize(&admin, &aureva_token, &event_config);
