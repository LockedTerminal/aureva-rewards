...
```bash
# from aureva-rewards/AurevaRewards/
docker build -t aureva-rewards-api ./backend
```
...
|---------|---------|-------------|
| api | 3001 | Aureva Rewards backend |
...
```bash
DATABASE_URL=postgresql://AUR:AUR@localhost:5432/aureva_rewards \
...
# Open a psql shell
docker compose exec postgres psql -U AUR -d aureva_rewards
