...
Aureva Rewards uses two complementary backup strategies:
...
- **S3 bucket**: `aureva-rewards-<env>-db-backups` — versioned, SSE-KMS, public access blocked
- **Lifecycle**: objects transition to STANDARD_IA after 7 days, expire after 30 days
- **KMS key**: `alias/aureva-rewards-<env>-backup`
- **SNS topic**: `aureva-rewards-<env>-backup-alerts` — receives failure notifications
- **IAM role**: `aureva-rewards-<env>-backup` — assumed by GitHub Actions via OIDC
...
| `DATABASE_URL` | `postgres://user:pass@host:5432/aureva_rewards` |
...
2. Restore via AWS Console:
   - RDS → Databases → `aureva-rewards-production` → Actions → **Restore to point in time**
   - Set **Custom date and time** to your target timestamp
   - Choose a new DB identifier (e.g. `aureva-rewards-production-restored`)
...
3. Or via AWS CLI:
   ...
   aws rds restore-db-instance-to-point-in-time \
     --source-db-instance-identifier aureva-rewards-production \
     --target-db-instance-identifier aureva-rewards-production-restored \
...
1. List available backups:
   ```bash
   aws s3 ls s3://aureva-rewards-production-db-backups/postgres/ \
...
2. Download the desired dump:
   ```bash
   aws s3 cp \
     s3://aureva-rewards-production-db-backups/postgres/aureva_rewards_<TIMESTAMP>.dump \
     ./aureva_rewards_restore.dump \
     --region us-east-1
   ```
...
```bash
# 1. Health endpoint
curl -f https://api.aureva-rewards.com/health
...
Critical alerts page via PagerDuty and post to `#AUR-critical` Slack. Warnings post to `#AUR-alerts`.
...
- [ ] Verify RDS PITR is available: check AWS Console → RDS → `aureva-rewards-production` → **Maintenance & backups** tab shows the expected retention window
