...
const INITIAL_SUPPLY = '1000000'; // 1,000,000 AUR
...
/**
 * Checks whether the Distribution Account already has a AUR trustline.
 *
 ...
 * 2. Establishes a AUR trustline on the Distribution Account (if not already set)
 * 3. Sends the initial AUR supply from Issuer to Distribution Account
 *
...
async function issueAsset() {
  ...
  console.log('=== aurevaRewards Asset Issuance ===');
  ...
  if (trustlineExists) {
    ...
    console.log('  Creating AUR trustline on Distribution Account...');
    ...
    const trustlineTx = new TransactionBuilder(distAccount, {
      ...
      .addOperation(Operation.changeTrust({ asset: AUR }))
      .setTimeout(180)
      .build();
  ...
  // Step 3: Send initial AUR supply from Issuer to Distribution Account
  // Check current balance first to stay idempotent
  console.log('\n[3] Checking Distribution Account AUR balance...');
  ...
  if (existingBalance && parseFloat(existingBalance.balance) > 0) {
    console.log(
      `  Distribution Account already holds ${existingBalance.balance} AUR — skipping initial supply.`
    );
  } else {
    console.log(`  Sending ${INITIAL_SUPPLY} AUR to Distribution Account...`);
