...
async function startEventListener() {
  const contracts = [aureva_token_CONTRACT_ID, REWARD_POOL_CONTRACT_ID].filter(Boolean);
...
async function dispatchEvent(contractId, eventType, raw, eventId) {
  switch (eventType) {
    ...
    // ── Token events ──────────────────────────────────────────────────────
    case 'aur_tok:burn':
    case 'aur_tok:transfer':
    case 'aur_tok:transfer_from':
    case 'aur_tok:approve':
    case 'aur_tok:inc_allow':
...
async function handleaurevaRewardsEvent(contractId, eventType, event, eventId) {
  logger.info(`[horizon-stream] aureva-rewards event type=${eventType} contract=${contractId} id=${eventId}`);
}
