...
async function sendRedemptionConfirmation({ to, userName, rewardName, pointsSpent, redemptionId }) {
  const subject = 'aurevaRewards - Redemption Confirmation';
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>Thank you for being a valued aurevaRewards member!</p>
        </div>
        <div class="footer">
          <p>&copy; ${new Date().getFullYear()} aurevaRewards. All rights reserved.</p>
        </div>
      </div>
    </body>
    </html>
...
async function sendMilestoneAchieved({ to, userName, milestoneName, rewardAmount }) {
  const subject = 'aurevaRewards - Milestone Achieved!';
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="milestone">
            <h3>${milestoneName}</h3>
            <p><strong>Reward:</strong> ${rewardAmount} AUR</p>
          </div>
...
async function sendWelcome({ to, userName, referralCode }) {
  const subject = 'Welcome to aurevaRewards!';
  ...
  const html = `
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>Welcome to aurevaRewards!</h1>
        </div>
        <div class="content">
          <p>Hi ${userName},</p>
          <p>Welcome to aurevaRewards! We're excited to have you on board.</p>
          <p>Start earning rewards by completing tasks, referring friends, and participating in campaigns.</p>
          ${referralSection}
          <p>Get started by exploring your dashboard and discovering all the ways to earn AUR tokens!</p>
        </div>
        <div class="footer">
          <p>&copy; ${new Date().getFullYear()} aurevaRewards. All rights reserved.</p>
        </div>
...
async function sendPasswordReset({ to, userName, resetLink }) {
  const subject = 'aurevaRewards - Password Reset Request';
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
