...
/**
 * Pact Provider Verification test for aureva-rewards-backend.
 *
 * Fetches all consumer contracts published to the Pact Broker by
 * aureva-rewards-frontend, replays each interaction against the running
 * Express server, and publishes the verification result back to the broker.
 *
 * Run with:
 *   PACT_BROKER_URL=https://pact.aurevaRewards.io \
 ...
 * contract file at aurevaRewards/frontend/pacts/ — the verifier will read
...
describe('Pact Provider Verification — aureva-rewards-backend', () => {
  let server;
  let port;
  ...
  it('verifies all consumer contracts from the Pact Broker', () => {
    ...
    if (isLocal) {
      ...
      verifierOptions.pactUrls = [
        path.resolve(
          __dirname,
          '../../frontend/pacts/aureva-rewards-frontend-aureva-rewards-backend.json'
        ),
      ];
    } else {
