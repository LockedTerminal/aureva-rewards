 ...
 *   - DB_NAME: Database name (default: aureva_rewards_test)
...
const DB_CONFIG = {
  ...
  database: process.env.DB_NAME || 'aureva_rewards_test',
  user: process.env.DB_USER || 'postgres',
  password: process.env.DB_PASSWORD || 'postgres',
};

// Test data constants
const TEST_MERCHANT_EMAIL = `merchant-test-${Date.now()}@test.aureva-rewards.com`;
  ...
  try {
    const result = await pool.query(
      ...
      [
        TEST_USER_WALLET,
        `Test User ${Date.now()}`,
        `user-${Date.now()}@test.aureva-rewards.com`,
        true,
        true,
      ]
    );
