...
describe('decodeContractResult', () => {
  ...
  test('round-trips a string value', () => {
    const b64 = nativeToScVal('AUR').toXDR('base64');
    expect(decodeContractResult(b64)).toBe('AUR');
  });
...
// ── aurevaRewardsContract wrappers ─────────────────────────────────────────────
describe('aurevaRewardsContract', () => {
  test('issueReward encodes 3 args', () => {
    const args = aurevaRewardsContract.issueReward(VALID_ADDRESS, 500n, 1n);
    expect(args).toHaveLength(3);
    expect(scValToNative(args[1])).toBe(500n);
  });

  test('redeem encodes 2 args', () => {
    const args = aurevaRewardsContract.redeem(VALID_ADDRESS, 100n);
    expect(args).toHaveLength(2);
  });

  test('decodeGetBalance round-trips bigint', () => {
    const b64 = nativeToScVal(250n).toXDR('base64');
    expect(aurevaRewardsContract.decodeGetBalance(b64)).toBe(250n);
  });
});

// ── AurevaTokenContract wrappers ───────────────────────────────────────────────
describe('AurevaTokenContract', () => {
  test('mint encodes to/amount', () => {
    const args = AurevaTokenContract.mint(VALID_ADDRESS, 1000n);
    expect(args).toHaveLength(2);
    expect(scValToNative(args[1])).toBe(1000n);
  });

  test('burn encodes from/amount', () => {
    const args = AurevaTokenContract.burn(VALID_ADDRESS, 50n);
    expect(scValToNative(args[1])).toBe(50n);
  });

  test('decodeBalance round-trips', () => {
    const b64 = nativeToScVal(777n).toXDR('base64');
    expect(AurevaTokenContract.decodeBalance(b64)).toBe(777n);
  });
});
