...
// =============================================================================
describe('Token transfer events (aur_tok)', () => {
  test('transfer — has topic array and structured data payload', () => {
    const event = mockRpcEvent('aur_tok', 'transfer', [ADDR_A, ADDR_B, 500]);
    expect(event.topic).toHaveLength(2);
    expect(event.topic[0].value).toBe('aur_tok');
