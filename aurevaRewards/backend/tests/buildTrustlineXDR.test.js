// Feature: aureva-rewards — buildTrustlineXDR
// Validates: Requirements 2.1
// Asserts:
//   1. server.loadAccount is mocked to return a mock account object
//   2. The returned XDR is a non-empty string
//   3. The transaction contains a changeTrust operation for the AUR asset
...
describe('buildTrustlineXDR', () => {
  ...
  test('XDR contains a changeTrust operation for the AUR asset', async () => {
    const xdrResult = await buildTrustlineXDR(walletAddress);
  ...
  test('transaction contains changeTrust operation for AUR asset', async () => {
    const xdrResult = await buildTrustlineXDR(walletAddress);
