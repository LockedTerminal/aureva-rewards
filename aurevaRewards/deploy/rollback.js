 ...
 * Rollback Script
 *
 * Attempts to reverse the effects of a Aureva Rewards deployment as much as
 * the Stellar network permits. Because Stellar is a live ledger, "rollback"
 * cannot delete accounts or transactions — it instead:
 *
 *   1. Burns remaining AUR tokens (Distribution → Issuer)
 *   2. Removes the AUR trustline from the Distribution Account
...
async function rollback({ network, deploymentId = null, dryRun = false }) {
  ...
  console.log('\n╔══════════════════════════════════════════════════════╗');
  console.log(`║   Aureva Rewards Rollback — ${cfg.name.padEnd(26)}║`);
  console.log('╚══════════════════════════════════════════════════════╝\n');
  ...
  const hasInitialSupply  = successSteps.has('Issue initial AUR supply');
  const hasTrustlineStep  = successSteps.has('Establish AUR trustline');
  const hasEnvStep        = successSteps.has('Validate environment');

  console.log('  Completed steps to reverse:');
  if (hasInitialSupply) console.log('    • Burn AUR tokens (send back to issuer)');
  if (hasTrustlineStep) console.log('    • Remove AUR trustline from Distribution Account');
  ...
  try {
    ...
    if (hasInitialSupply && !dryRun) {
      ...
      if (!hasTrustline || parseFloat(aurBalance) === 0) {
        ...
        log.markRolledBack('Issue initial AUR supply');
        console.log(`  ↩️   AUR burned. Tx: ${burnResult.hash}`);
      }
    } else if (hasInitialSupply && dryRun) {
      console.log('  [dry-run] Would burn AUR tokens from Distribution Account');
    }
    ...
    if (hasTrustlineStep && !dryRun) {
      ...
      if (!hasTrustline) {
        ...
        console.log('  Removing AUR trustline from Distribution Account...');
        ...
        log.markRolledBack('Establish AUR trustline');
        console.log(`  ↩️   Trustline removed. Tx: ${removeResult.hash}`);
      }
    } else if (hasTrustlineStep && dryRun) {
      console.log('  [dry-run] Would remove AUR trustline from Distribution Account');
    }
