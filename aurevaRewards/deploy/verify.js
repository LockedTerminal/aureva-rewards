 ...
 *   ✓ AUR asset code and issuer match expected values
 *   ✓ Distribution account has a AUR trustline
 *   ✓ Distribution account holds AUR tokens (> 0)
...
async function runVerification({ network, silent = false, deploymentId = null }) {
  ...
  const checks = await Promise.all([
    ...
    runCheck('Distribution account has AUR trustline', async () => {
      const account  = await server.loadAccount(distributionPublic);
