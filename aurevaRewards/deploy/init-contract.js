 ...
 *
 * Initializes the Aureva Rewards contract state after the core asset
 ...
 *   3. Verifies the initial AUR supply landed in the Distribution Account
...
/**
 * Fetches the current AUR balance for the Distribution Account.
 *
...
async function initContract({ network, logger = null, deploymentId = null }) {
  ...
  if (hasTrustline) {
    log.step({ name: 'Verify AUR trustline', status: Status.SUCCESS });
  } else {
    log.step({
      name:   'Verify AUR trustline',
      status: Status.FAILED,
      error:  'Distribution account is missing AUR trustline. Run the full deploy first.',
    });
    throw new Error('AUR trustline not found on Distribution Account.');
  }
  ...
  if (hasSupply) {
    ...
    log.step({
      name:   'Verify initial supply',
      status: Status.FAILED,
      error:  `Distribution account AUR balance is 0. Expected: ${cfg.initialSupply}`,
    });
    throw new Error('Initial AUR supply not found in Distribution Account.');
  }
