#!/usr/bin/env node
/**
 * Aureva Rewards — Main Deployment Script
 *
 * Orchestrates the full deployment of the Aureva Rewards token infrastructure
 * on either Stellar Testnet or Mainnet. Steps:
 *
 *   1. Validate environment
 *   2. Fund accounts (Testnet: Friendbot | Mainnet: manual pre-fund required)
 *   3. Establish AUR trustline on Distribution Account
 *   4. Issue initial AUR supply from Issuer to Distribution Account
...
/**
 * Checks whether an account has a AUR trustline.
 *
 * @param {Horizon.Server} server
 * @param {string} publicKey
 * @param {Asset}  asset
...
/**
 * Checks whether the Distribution Account already holds AUR tokens.
 *
 * @param {Horizon.Server} server
 * @param {string} publicKey
 * @param {Asset}  asset
...
async function deploy({ network, dryRun = false }) {
  ...
  // ── Banner ─────────────────────────────────────────────────────────────────
  console.log('\n╔══════════════════════════════════════════════════════╗');
  console.log(`║   Aureva Rewards Deployment — ${cfg.name.padEnd(24)}║`);
  console.log('╚══════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    if (hasTokens) {
      log.step({
        name:   'Issue initial AUR supply',
        status: Status.SKIPPED,
        data:   { reason: 'already_funded', currentBalance },
      });
      console.log(`  Distribution already holds ${currentBalance} AUR — skipped.`);
    } else {
      ...
      const paymentTx     = new TransactionBuilder(issuerAccount, {
        ...
        .addMemo(Memo.text('aurevaRewards initial supply'))
        .setTimeout(cfg.txTimeout)
        .build();
      ...
      console.log(`  ${cfg.initialSupply} AUR issued. Tx: ${paymentResult.hash}`);
    ...
    console.log('\n╔══════════════════════════════════════════════════════╗');
    console.log('║   ✅  Aureva Rewards deployed successfully!             ║');
    console.log('╚══════════════════════════════════════════════════════╝\n');
