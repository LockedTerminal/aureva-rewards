...
#   BACKUP_S3_BUCKET      — S3 bucket name (e.g. aureva-rewards-production-db-backups)
#
# Optional env vars:
...
FILENAME="aureva_rewards_${TIMESTAMP}.dump"
...
on_failure() {
  ...
  if [[ -n "${BACKUP_SNS_TOPIC_ARN:-}" ]]; then
    aws sns publish \
      ...
      --subject "Aureva Rewards DB Backup Failed" \
      --message "$msg" \
...
# ── Prune local backups ───────────────────────────────────────────────────────
find "$BACKUP_DIR" -name "aureva_rewards_*.dump" -mtime "+${RETAIN_DAYS}" -delete
