...
#   DB_NAME   — database name (aureva_rewards)
...
#   aureva-rewards/<env>/db-app-password
#   aureva-rewards/<env>/db-migrate-password
...
RDS_HOST=$(fetch_secret "aureva-rewards/${ENV_TAG}/rds-master" | python3 -c "import sys,json; print(json.load(sys.stdin)['host'])")
DB_NAME=$(fetch_secret  "aureva-rewards/${ENV_TAG}/rds-master" | python3 -c "import sys,json; print(json.load(sys.stdin)['dbname'])")
APP_PASS=$(fetch_secret "aureva-rewards/${ENV_TAG}/db-app-password")
MIG_PASS=$(fetch_secret "aureva-rewards/${ENV_TAG}/db-migrate-password")
