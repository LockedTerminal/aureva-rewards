...
resource "aws_cloudwatch_log_group" "service" {
  ...
  name              = "/aureva-rewards/${var.environment}/${each.key}"
...
resource "aws_cloudwatch_log_metric_filter" "backend_errors" {
  name           = "aureva-rewards-${var.environment}-backend-errors"
  log_group_name = aws_cloudwatch_log_group.service["backend"].name
  pattern        = "ERROR"

  metric_transformation {
    name          = "BackendErrorCount"
    namespace     = "AurevaRewards/${var.environment}"
    value         = "1"
    default_value = "0"
    unit          = "Count"
  }
...
resource "aws_cloudwatch_metric_alarm" "backend_error_rate" {
  alarm_name          = "aureva-rewards-${var.environment}-backend-error-rate"
